<template>
  <div class="grid grid-cols-6 max-md:grid-cols-2 gap-3 my-5 mx-auto">
    <div class="bg-[#161b22] border border-[#30363d] rounded-lg p-4" title="Wall-clock time from first event to last event in this session.">
      <div class="text-xs text-[#7d8590] uppercase tracking-[0.5px] mb-1">
Total Duration
</div>
      <div class="text-2xl font-semibold text-[#e6edf3]">
{{ formatDuration(totalDuration) }}
</div>
      <div v-if="sessionStart" class="text-xs text-[#7d8590] mt-0.5" style="font-size: 10px; opacity: 0.7;">
        {{ formatDateTime(sessionStart) }} → {{ formatDateTime(sessionEnd) }}
      </div>
    </div>
    <div class="bg-[#161b22] border border-[#30363d] rounded-lg p-4" title="Number of user messages that triggered agent work. Each request may involve multiple LLM turns.">
      <div class="text-xs text-[#7d8590] uppercase tracking-[0.5px] mb-1">
User Requests
</div>
      <div class="text-2xl font-semibold text-[#e6edf3]">
{{ groupedTurns.length }}
      ...
      <div class="text-xs text-[#7d8590] mt-0.5">
{{ turnAnalysis.length }} turn{{ turnAnalysis.length !== 1 ? 's' : '' }}
</div>
    </div>
    <div class="bg-[#161b22] border border-[#30363d] rounded-lg p-4" title="Total tool executions (Read, Write, Edit, Bash, Grep, etc.) across the entire session, including subagent tools.">
      <div class="text-xs text-[#7d8590] uppercase tracking-[0.5px] mb-1">
Tool Calls
</div>
      <div class="text-2xl font-semibold text-[#e6edf3]">
{{ totalToolCount }}
    ...
    <div class="bg-[#161b22] border border-[#30363d] rounded-lg p-4" title="Spawned subagents (via Task tool). Shows completed/failed/incomplete counts, total wall-clock time, and tool calls attributed to subagents.">
      <div class="text-xs text-[#7d8590] uppercase tracking-[0.5px] mb-1">
Sub-Agents
</div>
      <div class="text-2xl font-semibold text-[#e6edf3]">
{{ subagentAnalysis.length }}
      ...
      <div v-if="subagentAnalysis.length > 0" class="text-xs text-[#7d8590] mt-0.5">
    ...
    <div class="bg-[#161b22] border border-[#30363d] rounded-lg p-4" title="Estimated LLM reasoning time (total duration minus tool execution and user thinking time). Breakdown shows LLM percentage, tool wall-clock time, and user idle time.">
      <div class="text-xs text-[#7d8590] uppercase tracking-[0.5px] mb-1">
Time Breakdown
</div>
      <div class="text-2xl font-semibold text-[#e6edf3]">
{{ formatDuration(timeBreakdown.llmTime) }} <span style="font-size: 14px; opacity: 0.6;">({{ timeBreakdown.llmPct }}% LLM Reasoning)</span>
    ...
    <div class="bg-[#161b22] border border-[#30363d] rounded-lg p-4" title="Token usage across all models: input, output, and cached tokens.">
      <div class="text-xs text-[#7d8590] uppercase tracking-[0.5px] mb-1">
Token Usage
</div>
      <div class="text-2xl font-semibold text-[#e6edf3]">
