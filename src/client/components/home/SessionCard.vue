<template>
  <router-link :to="`/${urlSource}/session/${session.id}`" :class="['block bg-surface border border-border rounded-lg py-3 px-4 text-text-secondary no-underline transition-all min-h-[140px] overflow-hidden min-w-0 hover:border-accent hover:bg-surface-alt hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(88,166,255,0.2)]', session.sessionStatus === 'wip' ? 'border-warning border-l-[3px] border-l-warning hover:border-warning hover:border-l-[#e8b634]' : '']">
    ...
    <div class="flex flex-wrap items-center justify-start gap-1 mt-1">
      <div class="flex flex-wrap gap-1">
        <span :class="['inline-block text-base align-middle opacity-80 transition-opacity duration-200 hover:opacity-100 py-0.5 px-2 rounded-xl text-2xs font-semibold font-mono', sourceBadgeStyle.classes]" :style="sourceBadgeStyle.style" :title="session.sourceName || 'Copilot'">{{ session.sourceName || 'Copilot' }}</span>
        <span v-if="session.sessionStatus === 'wip'" class="inline-block text-base align-middle opacity-80 transition-opacity duration-200 hover:opacity-100 py-0.5 px-2 rounded-xl text-2xs font-semibold bg-[rgba(210,153,34,0.2)] text-warning border border-[rgba(210,153,34,0.4)]" title="Session in progress">🔄 WIP</span>
        <span v-if="session.isImported" class="inline-block text-base align-middle opacity-80 transition-opacity duration-200 hover:opacity-100" title="Imported session">📥</span>
        <span v-if="session.hasInsight" class="inline-block text-base align-middle opacity-80 transition-opacity duration-200 hover:opacity-100" title="Has Agent Review">💡</span>
        <span v-if="session.selectedModel" class="inline-block text-base align-middle opacity-80 transition-opacity duration-200 hover:opacity-100 py-0.5 px-2 rounded-xl text-2xs font-semibold font-mono" :style="modelStyle" :title="`Model: ${session.selectedModel}`">{{ modelShort }}</span>
        <span v-if="session.source === 'modernize' && session.modernizeVersion" class="inline-block text-base align-middle opacity-80 transition-opacity duration-200 hover:opacity-100 py-0.5 px-2 bg-[rgba(234,179,8,0.15)] text-warning rounded-xl text-2xs font-semibold font-mono" title="Modernize version">{{ session.modernizeVersion }}</span>
        <span v-else-if="session.copilotVersion" class="inline-block text-base align-middle opacity-80 transition-opacity duration-200 hover:opacity-100 py-0.5 px-2 bg-[rgba(234,179,8,0.15)] text-warning rounded-xl text-2xs font-semibold font-mono" title="CLI version">{{ session.copilotVersion }}</span>
        <span v-if="session.agentName" class="inline-block text-base align-middle opacity-80 transition-opacity duration-200 hover:opacity-100" :title="`Agent: ${session.agentName}`">🤖 {{ session.agentName }}</span>
      </div>
...
No summary available
    ...
    <div class="flex items-center gap-4 flex-wrap text-xs font-mono min-w-0">
      <div v-if="session.workspace && session.workspace.cwd" class="flex items-center gap-1.5 min-w-0 text-text-faint basis-full" :title="session.workspace.cwd">
      ...
      <div class="flex items-center gap-1.5 min-w-0 text-text-faint">
      ...
      <div v-if="session.duration" class="flex items-center gap-1.5 min-w-0 text-text-faint">
